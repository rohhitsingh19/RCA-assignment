...
- **Antigravity (Gemini-powered AI assistant)** — used for debugging dependency conflicts (fastapi/mcp starlette version mismatch), fixing mismatched filenames (CSV and doc files had `amazon_` prefix in code but `quick_commerce_` on disk), adding missing dependencies (numpy, pandas for Python 3.14 compatibility), and performance optimization (caching the MCP system context).
- **Claude (claude.ai)** — used during initial architecture design and code generation for all modules.
- All generated code was reviewed, tested, and understood before submission. No code was copy-pasted blindly — every function was read and verified against the RCA playbook.
